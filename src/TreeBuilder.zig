...
pub fn serializeTree(ast: *const Ast, output: *std.ArrayList(u8), allocator: Allocator) !void {
    ...
    if (doc_idx) |idx| {
        ...
        for (children, 0..) |child_idx, i| {
            ...
            try serializeNode(ast, child_idx, writer);
        }
    }
...
fn serializeNode(ast: *const Ast, node_idx: Ast.NodeIndex, writer: anytype) !void {
    ...
    switch (node.tag) {
        ...
        .link, .image => {
            ...
            if (link.text_node != 0) {
                try writer.writeAll(",\"children\":[");
                try serializeNode(ast, link.text_node, writer);
                try writer.writeAll("]");
            } else {
                try writer.writeAll(",\"children\":[]");
            }
